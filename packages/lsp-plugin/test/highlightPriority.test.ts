 ...
 * | `FIND_CURRENT_STYLE`       | 3        | an answer to a question the user just asked          |
 *
...
describe('the highlight priority band', () => {
  ...
  it('pins the numbers themselves, so a change has to be deliberate', () => {
    ...
    expect(FIND_HIGHLIGHT_Z_INDEX.current).toBe(3)
  })

  /**
   * The two find styles that share numbers with the semantic layer and the error diagnostic declare
   * only a background, so they never contend with either for `color`. That is why they were left
   * alone rather than renumbered around the new band.
   */
  it('leaves the background-only find styles where they are', () => {
    expect(FIND_HIGHLIGHT_Z_INDEX.scope).toBe(SEMANTIC_TOKEN_Z_INDEX)
    expect(FIND_HIGHLIGHT_Z_INDEX.match).toBe(DIAGNOSTIC_STYLES.error.zIndex)
  })
