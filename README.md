...
`core` (the `editor` folder), `gutters`, `find`, `markdown`, `minimap`, `scope-lines`, `diff`, `panes`, `tree-sitter`, `tree-sitter-languages`, `lsp`, `lsp-plugin`, `typescript-lsp`, `decode`, `react`, `solid`. all under `@singapore-editor/`, one folder each in `packages/`
